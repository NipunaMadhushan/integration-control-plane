...
configurable string defaultApiKey = "default-api-key";
...
public http:Service opensearchService = service object {
    ...
    resource function post logs(@http:Header {name: "X-API-Key"} string? apiKeyHeader, @http:Payload types:LogEntryRequest logRequest) returns types:LogEntriesResponse|http:BadRequest|error {       
        // Validate API key
        if apiKeyHeader is () || apiKeyHeader != defaultApiKey {
        ...
        map<json> baseQuery = check buildBaseQuery(logRequest);
...
isolated function buildBaseQuery(types:LogEntryRequest logRequest) returns map<json>|error {
    map<json> query = {
        "query": {
            "bool": {
                "must": [],
    ...
    string|string[]? runtimeValue = logRequest.runtime;
    ...
    } else if runtimeValue is string[] {
        json runtimeFilter = {
            "terms": {
                "runtime.keyword": runtimeValue
            }
    ...
    string|string[]? componentValue = logRequest.component;
    ...
    } else if componentValue is string[] {
        json componentFilter = {
            "terms": {
                "component.keyword": componentValue
            }
    ...
    string|string[]? environmentValue = logRequest.environment;
    ...
    } else if environmentValue is string[] {
        json environmentFilter = {
            "terms": {
                "environment.keyword": environmentValue
            }
    ...
    string|string[]? projectValue = logRequest.project;
